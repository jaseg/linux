...
struct gdepaper {
	...
	enum gdepaper_vghl_lv vg_lv; /* gate voltage level */
	bool vds_en; /* Internal source voltage enable */
	bool vdg_en;  /* Internal gate voltage enable */
	u32 vcom_sel;
	s32 vdh_bw_mv; /* drive high level, b/w pixel (mV) */
	s32 vdh_col_mv; /* drive high level, red/yellow pixel (mV) */
	s32 vdl_mv; /* drive low level (mV) */
	s32 vcom_dc_mv;
	u8 ss_param[3]; /* boost converter soft start parameter */
	u32 vcom_data_ivl_hsync; /* hsync periods */
	u32 border_data_sel; /* "vbd" */
					  ...
					  size_t len)
{
	...
	for (i=0; i<len; i++) {
		ret = tinydrm_spi_transfer(epap->mipi.spi, epap->spi_speed_hz,
					   NULL, 8, &data[i], 1);
		if (ret)
			return ret;
...
static int gdepaper_command(struct gdepaper *epap, u8 cmd,
				   u8 *par, size_t num)
{
	struct mipi_dbi *mipi = &epap->mipi;
	struct spi_device *spi = mipi->spi;
	int ret;
	u8 cmd_buf = cmd;
	dev_dbg(epap->mipi.drm.dev, "tx: cmd=0x%x len=%zu buf=%*ph\n",
		cmd, num, (int)num, par);

	gpiod_set_value_cansleep(mipi->dc, 0);
	ret = tinydrm_spi_transfer(spi, epap->spi_speed_hz, NULL, 8,
				   &cmd_buf, 1);
	if (ret || !num)
		return ret;
...
static inline struct gdepaper *mipi_dbi_to_gdepaper(struct mipi_dbi *mipi)
{
	return container_of(mipi, struct gdepaper, mipi);
}
...
static int gdepaper_wait_busy(struct gdepaper *epap)
{
	struct mipi_dbi *mipi = &epap->mipi;
	int i = 18000;
	dev_dbg(mipi->drm.dev, "waiting for busy line\n");
	while (i--) {
...
static int gdepaper_power_off(struct gdepaper *epap)
{
	...
	dev_dbg(epap->mipi.drm.dev, "display power off\n");
...
static int gdepaper_enter_deep_sleep(struct gdepaper *epap)
{
	u8 param = 0xa5;
	epap->is_powered_on = false;
	dev_dbg(epap->mipi.drm.dev, "display deep sleep\n");
...
{
	struct device *dev = epap->mipi.drm.dev;
	int ret;
	ret = mipi_dbi_command_buf(&epap->mipi, GDEP_CMD_PWR_ON, NULL, 0);
	if (ret)
		return ret;
...
static void gdepaper_reset(struct gdepaper *epap) {
	gpiod_set_value_cansleep(epap->mipi.reset, 0);
	usleep_range(10000, 20000);
	gpiod_set_value_cansleep(epap->mipi.reset, 1);
	msleep(200);
}
				...
				u8 cmd, u8 *buf, size_t len)
{
	...
	dev_dbg(epap->mipi.drm.dev, "Running partial command 0x%x on rect "
	...
	dev_dbg(epap->mipi.drm.dev, "partial payload: len=%zu buf=%*ph\n",
		len, (int)len, buf);
...
static void gdepaper_fb_dirty(struct drm_framebuffer *fb, struct drm_rect *rect)
{
	...
	if (!mipi->enabled) {
		dev_dbg(dev, "mipi is disabled, returning\n");
		return;
	}
	...
	if (w == fb->width && h == fb->height) { /* full refresh */
		/* black */
		ret = gdepaper_txbuf_pack((u8*)mipi->tx_buf, fb, rect,
					  GDEP_CH_BLACK);
		...
		/* black */
		ret = gdepaper_txbuf_pack((u8*)mipi->tx_buf, fb, &rect_aligned,
					  GDEP_CH_BLACK);
		dev_dbg(dev, "Sending %d byte partial framebuf\n", ret);
		if (ret < 0)
			goto err_out;
		ret = gdepaper_partial_cmd(epap, &rect_aligned,
			GDEP_CMD_PD_START_TX_COL1, (u8*)mipi->tx_buf, ret);
		if (ret)
			goto err_out;
		...
		if (ret < 0)
			goto err_out;
		ret = gdepaper_partial_cmd(epap, &rect_aligned,
			GDEP_CMD_PD_START_TX_COL2, (u8*)mipi->tx_buf,
			ret);
		if (ret)
			goto err_out;
		ret = gdepaper_command(epap, GDEP_CMD_DATA_STOP, NULL, 0);
				 ...
				 struct drm_plane_state *plane_state)
{
	...
	struct {
		...
		.vcom_hv = !!epap->vcom_sel, .vg_lv = epap->vg_lv,
		.vdh = (epap->vdh_bw_mv-2400)/200,
		.vdl = (epap->vdl_mv-2400)/200,
		.vdhr = (epap->vdh_col_mv-2400)/200,
	};
	 ...
	 * coefficient table. */
	switch (epap->pll_div) {
		case 1: param = 0; fps_min = 68010; fps_max=197610; break;
		case 2: param = 1; fps_min = 34010; fps_max=120860; break;
		case 4: param = 2; fps_min = 20450; fps_max=60430; break;
		case 8: param = 3; fps_min = 20220; fps_max=30220; break;
		default:
		dev_err_once(dev, "Invalid pll_div %d\n", epap->pll_div);
		goto err_out;
	}
	if (epap->framerate_mHz < fps_min || epap->framerate_mHz > fps_max) {
		dev_err_once(dev, "Framerate %d out of range for pll_div %d"
				  " (%d-%d)\n",
	 ...
	 */
	foo = (epap->framerate_mHz*1000 - (68014451 / epap->pll_div)) / (2757368 * epap->pll_div);
	if (foo < 0 || foo > 63) {
		dev_err_once(dev, "PLL multiplier for framerate %d and "
				  "pll_div %d out of range\n",
				   epap->framerate_mHz, epap->pll_div);
		goto err_out;
	}
	...
	step = 3;
	if (epap->vg_lv < 0 || epap->vg_lv > 3) {
		dev_err_once(dev, "Invalid vg_lv value %d\n", epap->vg_lv);
		goto err_out;
	}
	if (epap->vdh_bw_mv < 2400 || epap->vdh_bw_mv > 11000) {
		dev_err_once(dev, "vdh_bw=%d out of range (2.4V - 11.0V)\n",
			     epap->vdh_bw_mv);
		goto err_out;
	}
	if (epap->vdl_mv < 2400 || epap->vdl_mv > 11000) {
		dev_err_once(dev, "vdl_mv=%d out of range (-11.0V - -2.4V)\n",
			     epap->vdl_mv);
		goto err_out;
	}
	if (epap->vdh_col_mv < 2400 || epap->vdh_col_mv > 11000) {
	...
	for (i=0; i<ARRAY_SIZE(pwr_opt); i++) {
		/* FIXME check in pulseview this endianness conversion works as intended */
	...
	step = 10;
	param = (epap->vcom_dc_mv - 100) / 50;
	ret = gdepaper_command(epap, GDEP_CMD_VDC_SET, &param, sizeof(param));
	if (ret)
		goto err_out;

	step = 11;
	if (epap->vcom_data_ivl_hsync < 2 || epap->vcom_data_ivl_hsync > 17) {
		dev_err_once(dev, "Invalid vcom/data ivl setting %d "
				  "(should be 2-17)\n",
				  epap->vcom_data_ivl_hsync);
		goto err_out;
	}
	if (epap->border_data_sel < 0 || epap->border_data_sel > 3) {
		dev_err_once(dev, "Invalid border_data_sel (vbd) setting %d\n",
				  epap->border_data_sel);
		goto err_out;
	}
	if (epap->data_polarity < 0 || epap->data_polarity > 3) {
		dev_err_once(dev, "Invalid data polarity (ddx) setting %d\n",
				  epap->border_data_sel);
		goto err_out;
	}
	param = epap->border_data_sel<<6 | epap->data_polarity<<4;
	param |= epap->vcom_data_ivl_hsync;
	ret = gdepaper_command(epap, GDEP_CMD_VCOM_DIVL_SET, &param, sizeof(param));
	if (ret)
		goto err_out;

	if (epap->luts) {
		step = 12;
		/* Use default LUTs for now. TODO: Read from device tree? */
	...
	mipi->enabled = true;
	gdepaper_fb_dirty(plane_state->fb, &rect);
...
static void gdepaper_pipe_disable(struct drm_simple_display_pipe *pipe)
{
	struct mipi_dbi *mipi = drm_to_mipi_dbi(pipe->crtc.dev);
	struct gdepaper *epap = mipi_dbi_to_gdepaper(mipi);
	dev_dbg(mipi->drm.dev, "Disabling gdepaper pipe\n");
...
static int gdepaper_of_read_luts(struct gdepaper *epap, struct device_node *np,
	...
	ret[0] = of_property_read_u8_array(np, "lut_vcom_dc",
	       	epap->luts->lut_vcom_dc, sizeof(epap->luts->lut_vcom_dc));
	ret[1] = of_property_read_u8_array(np, "lut_ww",
	       	epap->luts->lut_ww, sizeof(epap->luts->lut_ww));
	ret[2] = of_property_read_u8_array(np, "lut_wb",
	       	epap->luts->lut_wb, sizeof(epap->luts->lut_wb));
	ret[3] = of_property_read_u8_array(np, "lut_bw",
	       	epap->luts->lut_bw, sizeof(epap->luts->lut_bw));
	ret[4] = of_property_read_u8_array(np, "lut_bb",
			epap->luts->lut_bb, sizeof(epap->luts->lut_bb));
...
MODULE_DEVICE_TABLE(spi, gdepaper_id);

static const u8 gdepaper_dcs_read_commands[] = {
	0, /* sentinel */
};
...
static int gdepaper_probe(struct spi_device *spi)
{
	...
	epap->vg_lv = GDEP_PWR_VGHL_16V;
	epap->vcom_sel = 0;
	epap->vdh_bw_mv = 11000; /* drive high level, b/w pixel */
	epap->vdh_col_mv = 4200; /* drive high level, red/yellow pixel */
	epap->vdl_mv = -11000; /* drive low level */
	epap->border_data_sel = 2; /* "vbd" */
	epap->data_polarity = 0; /* "ddx" */
	epap->vcom_dc_mv = -1000;
	epap->ss_param[0] = 0x07;
	epap->ss_param[1] = 0x07;
	epap->ss_param[2] = 0x17;
	epap->controller_res = GDEP_CTRL_RES_320X300;
	epap->vcom_data_ivl_hsync = 10; /* hsync periods */

	of_property_read_u32(np, "controller-resolution",
			&epap->controller_res);
	of_property_read_u32(np, "spi_speed_hz", &epap->spi_speed_hz);
	...
	of_property_read_u32(np, "vghl-level", &epap->vg_lv);
	epap->vds_en = !of_property_read_bool(np, "vds-external");
	epap->vdg_en = !of_property_read_bool(np, "vdg-external");
	of_property_read_u32(np, "vcom", &epap->vcom_sel);
	of_property_read_u32(np, "vdh-bw-millivolts", &epap->vdh_bw_mv);
	of_property_read_u32(np, "vdh-color-millivolts", &epap->vdh_col_mv);
	of_property_read_u32(np, "vdl-millivolts", &epap->vdl_mv);
	of_property_read_u32(np, "border-data", &epap->border_data_sel);
	of_property_read_u32(np, "data-polarity", &epap->data_polarity);
	...
	if (epap->vdl_mv < 0)
		epap->vdl_mv = -epap->vdl_mv;
	if (epap->vcom_dc_mv < 0)
		epap->vcom_dc_mv = -epap->vcom_dc_mv;
	...
	ret = mipi_dbi_init(mipi, &gdepaper_pipe_funcs, mode, 0);
	if (ret)
		return ret;
